...
*/
void UsartTransmit(Intmash_Usart* UserUsartStr, u8* Buffer, u8 Cnt){//настройка DMA на передачу данных в UART
  //SetDIRToTX;
...
*/
void UsartRecieve(Intmash_Usart* UserUsartStr, u8* Buffer){//настройка DMA на чтение данных из UART
...
u8 UsartTxRxFinish(Intmash_Usart* UserUsartStr){
  ...
  // Передача окончена (последний байт полностью передан в порт)
  if((IIR & USART_ISR_TC) && (UserUsartStr->USARTx->CR1 & USART_CR1_TCIE)){
    USART_ClearFlag(UserUsartStr->USARTx, USART_ICR_TCCF);//сбросить флаг окончания передачи 
  ...
  // Между байтами при приёме обнаружена пауза в 1 IDLE байт
  if((IIR & USART_ISR_IDLE) & (UserUsartStr->USARTx->CR1 & USART_CR1_IDLEIE)){
