...
std::string getVarStrValueByKey(std::string& key, const char* dev) {
	...
	std::string sval = "1.0";//IniResources::getScaleValueByKey(key, _dev);
	return sval;
}

float ScaleUtils::getScaleFromProps(const char* dev, const char* optional) {
	char* pScale = IniParser::getInstance().getElementPtrByNumber(3, '/', (char*) optional);
	std::string s = "1.0";//IniParser::getInstance().getElement('/', pScale); //TODO обработка исключений
