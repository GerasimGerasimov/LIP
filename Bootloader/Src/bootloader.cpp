...
#define STEP_SIZE                   0x800
#define START_ADDRESS               0x08000000
#define PAGE_SIZE_VAL               (STEP_SIZE - 1) // Автоматический расчет

// Функция для добавления HEX-адреса в буфер (без sprintf)
char* append_hex(char* ptr, uint32_t val){
    const char hex_chars[] = "0123456789ABCDEF";
    for (int i = 7; i >= 0; i--) {
        ptr[i] = hex_chars[val & 0xF];
        val >>= 4;
    }
    return ptr + 8;
}

// Универсальная функция добавления десятичного числа
char* append_int(char* ptr, uint32_t val) {
    char temp[10]; // Достаточно для 32-битного числа (до 4 млрд)
    int i = 0;

    // Обрабатываем случай с 0
    if (val == 0) {
        *ptr++ = '0';
        return ptr;
    }
...
// Вспомогательная функция для копирования строковых литералов
char* append_str(char* ptr, const char* str) {
    while (*str) *ptr++ = *str++;
    return ptr;
}

//Не используется sprintf() для экономии flash памяти
u16 getPagesList(TClient* Slave){
    u16 DataLength = 0;

    char *ptr = (char*)&Slave->Buffer[BOOT_PAGES_LIST_HEAD_SIZE];
    u32 current_addr = START_ADDRESS;

    // 1. Ставим открывающую скобку массива
    *ptr++ = '[';

    for (int i = 0; i < PAGE_COUNT; i++){
        ptr = append_str(ptr, "{\"start\": \"0x");
        ptr = append_hex(ptr, current_addr);
        ptr = append_str(ptr, "\", \"size\": ");
        ptr = append_int(ptr, PAGE_SIZE_VAL);
        *ptr++ = '}';
    ...
    // 4. Ставим закрывающую скобку и терминирующий ноль
    *ptr++ = ']';
    *ptr = '\0';

    // 5. Вычисляем итоговую длину (разница между текущим и начальным указателем)
    DataLength = (u16)(ptr - (char*)&Slave->Buffer[BOOT_PAGES_LIST_HEAD_SIZE]);
